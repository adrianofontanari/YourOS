...
-----
> [!todo]
> Guide: replace1-2 = monday-monday dates of the week
...
FROM #journal/daily
WHERE daily-recap AND file.ctime >= date(replace1) AND file.ctime <= date(replace2)
SORT file.ctime Asc
```
```custom-frames
...
WHERE life-learning AND file.ctime >= date(replace1) AND file.ctime <= date(replace2)
...
WHERE file.ctime >= date(replace1) AND file.ctime <= date(replace2)
SORT file.ctime Asc
```
#### Health
- 
...
WHERE medication AND file.ctime >= date(replace1) AND file.ctime <= date(replace2)
SORT file.ctime DESC
```
```dataview TABLE 
TABLE WITHOUT ID (link(file.path)) as Day, symptom as Symptoms FROM #journal/daily
WHERE symptom AND file.ctime >= date(replace1) AND file.ctime <= date(replace2)
SORT file.ctime DESC
```
...
WHERE praise AND file.ctime >= date(replace1) AND file.ctime <= date(replace2)
SORT file.ctime DESC
```
#### Identity:
- 
...
WHERE memory AND file.ctime >= date(replace1) AND file.ctime <= date(replace2)
SORT file.ctime DESC
```

```dataview TABLE 
TABLE WITHOUT ID (link(file.path)) as Day, grateful as Grateful FROM #journal/daily
WHERE grateful AND file.ctime >= date(replace1) AND file.ctime <= date(replace2)
SORT file.ctime DESC
```
...
```dataview
TASK 
WHERE completion >= date(replace1) AND completion < date(replace2)
```
